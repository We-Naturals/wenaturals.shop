-- 1. Secure Orders Table (Prevent Status Manipulation)
-- Revoke the unsafe "update" policy
DROP POLICY IF EXISTS "Users can update own orders" ON public.orders;

-- Add Read-Only policy for users (if not already present)
DROP POLICY IF EXISTS "Users can view own orders" ON public.orders;
CREATE POLICY "Users can view own orders" 
ON public.orders FOR SELECT 
USING (auth.uid() = user_id);

-- Note: We generally DO NOT want users to update orders directly. 
-- Payment updates come via Service Role (Webhook/API).
-- Cancellations should be a specific RPC or strict policy.
-- For now, we allow NO updates from the client for maximum security.

-- 2. Secure RPC (Prevent ID Spoofing)
